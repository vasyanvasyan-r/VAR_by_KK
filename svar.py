...
class SVAR_KL:
    ...
    def identify_combined(
        ...
        def penalty(params: np.ndarray) -> float:
            ...
            B0inv = self.P @ Q
            Upsilon = _long_run_matrix(self.A_endo_no_const, B0inv) # type: ignore
            ...
            if short_sign_restrictions:
                ...
                irfs = _irf_companion(self.A_endo_no_const, B0inv, horizon=max_h) # type: ignore
                for (r, c, h, sgn) in short_sign_restrictions:
                    v = irfs[h, r, c]
                    # smooth asymmetric penalty: exp(-sgn*v)-1
                    loss += float(np.exp(-sgn * v))
